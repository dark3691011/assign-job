...
export class QueueProcessor {
  ...
  async handleAssignTask(job: Job) {
    ...
    try {
      ...
      this.queueService.cc({ taskId, userId });
    } catch (error) {
      console.error(
  ...
  async handlePendingQueue() {
    ...
    await this.queueService.assignPendingTaskss();
  }
  ...
  private async updateTaskStatus(
    ...
    if (randomValue < 0.3) {
      ...
      const error = new Error(
        isUserError
          ? 'Simulated user-related failure'
          : 'Simulated task-related failure',
      );
    ...
    console.log(
      `Task ${taskId} updated to status "${status}" for user ${userId}`,
    );
  }
}
